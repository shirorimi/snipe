...
Window_ActorCommand.prototype.load_com_images = function() {
	...
	for (var i = 0; i < this._list.length; i++) {
		 ...
		 for (var r = 0; r < this._list.length; r++) {
                      if(this._list[r].name=="隠れる"){tmp = "kakureru"}
                      if(this._list[r].name=="騙す"){tmp = "damasu"}
	              if(this._list[r].name=="耐える"){tmp = "taeru"}
	              if(this._list[r].name=="アイテム"){tmp = "item"}
	              if(this._list[r].name=="狙撃"){tmp = "sogeki"}
		      this._com_images.push(ImageManager.loadBcom("Com_" + tmp));
		 };
	};
